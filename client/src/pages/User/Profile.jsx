...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchExaminee = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/examinee/${examineeId}`);
        console.log(res.data.data);
        
        if (res.data) {
          setFormData(res.data.data);
          if (res.data.profileImage) {
            setProfilePic(`${import.meta.env.VITE_API_BASE_URL}/uploads/${res.data.profileImage}`);
          }
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/examinee/${examineeId}`,
        data,
