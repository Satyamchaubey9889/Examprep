...
const ContactA = () => {
  ...
  const fetchUserMessages = async () => {
    if (!userId) return;
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/message/user/${userId}`);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/message`, { question, examineeId: userId });
      setQuestion('');
      fetchUserMessages();
    } catch (err) {
  ...
  const editMyMessage = async (id, currentText) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/message/edit/${id}`, {
        question: newText,
        role: 'user',
        userId
      });
  ...
  const deleteByUser = async (id) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/message/delete/${id}`, {
        role: 'user',
        userId
      });
      fetchUserMessages();
