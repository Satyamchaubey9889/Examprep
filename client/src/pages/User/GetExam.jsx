...
const GetExam = () => {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/exams/exam/${examId}`);
  ...
  const handleSubmit = async () => {
    if (submitted) return;
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/exams/submit-exam`, {
        examId,
        answers,
        email,
      });
