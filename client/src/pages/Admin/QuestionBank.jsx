...
const QuestionBank = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editform) {
        const res = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/question/${id.id}`, formData);
        if (res) {
          alert('Question updated successfully');
        }
      } else {
        const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/question`, formData);
        if (res) {
  ...
  const handlefetch = async () => {
    const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/question`);
    setData(res.data.data);

    const res1 = await axios.get(`${ import.meta.env.VITE_API_BASE_URL } / api / subject`);
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`${ import.meta.env.VITE_API_BASE_URL } / api / question / ${ id }`);
      if (res) {
