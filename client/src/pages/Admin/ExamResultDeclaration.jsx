...
const ExamResultsDeclaration = ({ exams }) => {
  ...
  const handleDeclare = async (examResultId) => {
    ...
    setMessage(null);
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/exams/result/${examResultId}`);
  ...
  const handlefetch = async () => {
    const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/exams/examination`)
