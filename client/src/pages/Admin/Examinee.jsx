...
const Examinee = () => {
  ...
  const handlefetch = async () => {
    const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/examinee`);
    setData(res.data.data);
  };

  const handleDelete = async (id) => {
    const res = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/examinee/${id}`);
    if (res) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editingId) return;
    try {
      await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/examinee/${editingId}`, form);
